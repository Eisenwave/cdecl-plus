...
export class Explainer {
    ...
    declarationsToProse(ast) {
        ...
        for (const declarator of ast.declarators) {
            const paragraph = this.declarationWithKnownSpecifiersToProse(specifiersProse, declarator, 'top-level')
                .replace(/ +/g, ' ');
    ...
    declarationToProse(specifiers, declarator, kind) {
        return this.declarationWithKnownSpecifiersToProse(this.specifiersToProse(specifiers, kind), declarator, kind);
    }
    ...
    specifiersToProse(specifiers, kind) {
        ...
        let atomicProse = atomicSpecifier !== undefined ?
            'atomic ' + this.declarationToProse(atomicSpecifier[1].specifiers, atomicSpecifier[1].declarator, 'atomic') : '';

        return {
            outer: this.processedSpecifiersToText(specifiers.filter(s => OUTER_SPECIFIER_TYPES.has(s[0]))),
            inner: this.processedSpecifiersToText(specifiers.filter(s => INNER_SPECIFIER_TYPES.has(s[0]))),
            atomic: atomicProse,
            histogram: histogram
        };
    }
     ...
     * @param a {string} the first specifier
     * @param b {string} the second specifier
                ...
                case '()': {
                    ...
                    for (const p of d.params) {
                        if (p.typ === '...') {
                            ...
                            paramsProses.push(this.declarationToProse(p.specifiers, p.declarator, 'parameter'));
                        }
                    }
